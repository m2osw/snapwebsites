<?xml version="1.0"?>
<!--
Snap Websites Server == default record body parser
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:param name="layout-name">default-record</xsl:param>
	<xsl:param name="layout-area">default-record-body</xsl:param>
	<xsl:param name="layout-modified">2019-03-04 10:19:12</xsl:param>
	...
	<xsl:template match="snap">
		<output>
			<li>
				<xsl:attribute name="class">index-record index<xsl:value-of select="@index"/><xsl:if test="@index mod 2 = 0"> even</xsl:if><xsl:if test="@index mod 2 = 1"> odd</xsl:if></xsl:attribute>
				<a href="/{$full_path}">
					<xsl:attribute name="class">index-record-anchor
