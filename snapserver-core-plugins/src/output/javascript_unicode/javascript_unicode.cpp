...
int                 g_year = 2016; // we started in 2016 or so to implement this file
...
void output_header()
{
    ...
    g_out << "/** @preserve" << std::endl
          ...
          << " * Copyright: Copyright 2016-"
                 << g_year
                 << " (c) Made to Order Software Corporation  All right reserved." << std::endl
          << " * License: GPL 2.0" << std::endl
          << " * Description: WARNING -- this code is generated by javascript_unicode.cpp" << std::endl
          << " *                         it is part of Snap! Websites (https://snapwebsites.org/)" << std::endl
          << " */" << std::endl;
...
{
    {
        ...
        if(strftime(buf, sizeof(buf), "%Y", now) == 0)
        {
            std::cerr << "fatal error: could not retrieve current year." << std::endl;
            exit(1);
        }
        g_year = std::stoi(buf);
    }
